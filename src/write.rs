...
use mech3rs::archive::{Entry, Mode, Version};
...
fn write_reader_transform(name: &str, data: Vec<u8>) -> Result<Vec<u8>> {
    ...
    let mut buf = Vec::new();
    mech3rs::reader::write_reader(&mut buf, &value)
...
fn write_motion_transform(name: &str, data: Vec<u8>) -> Result<Vec<u8>> {
    let motion: mech3rs::motion::Motion = serde_json::from_slice(&data)
        .with_context(|| format!("Motion data for \"{}\" is invalid", name))?;

    let mut buf = Vec::new();
    mech3rs::motion::write_motion(&mut buf, &motion)
...
fn write_mechlib_transform(name: &str, data: Vec<u8>) -> Result<Vec<u8>> {
    match name {
        ...
        "version" => Ok(data),
        "materials" => {
            let materials: Vec<mech3rs::mechlib::Material> =
                serde_json::from_slice(&data).context("Materials data is invalid")?;

            let mut buf = Vec::new();
            mech3rs::mechlib::write_materials(&mut buf, &materials)
                .context("Failed to write materials data")?;
            Ok(buf)
        }
        original => {
            let mut model: mech3rs::mechlib::Model = serde_json::from_slice(&data)
                .with_context(|| format!("Model data for \"{}\" is invalid", original))?;

            let mut buf = Vec::new();
            mech3rs::mechlib::write_model(&mut buf, &mut model)
...
fn parse_manifest(ptr: *const u8, len: usize) -> Result<mech3rs::textures::Manifest> {
    if ptr.is_null() {
...
pub extern "C" fn write_textures(
    ...
    err_to_c(|| {
        ...
        let mut write = buf_writer(filename)?;
        mech3rs::textures::write_textures(&mut write, &manifest, |name| {
            let mut buffer = CallbackBuffer::new();
            let ret = callback(name.as_ptr(), name.len(), &mut buffer);
            if ret != 0 {
                bail!("callback returned {} on \"{}\"", ret, name);
