...
pub(crate) fn textures(opts: JsonOpts) -> Result<()> {
    ...
    let mut texture_infos: Vec<TextureInfo> = serde_json::from_slice(&buf)?;
    let parent = path.parent().expect("Manifest path must have a parent");

    let mut images: HashMap<String, DynamicImage> = texture_infos
        .iter_mut()
        .map(|info| {
    ...
    write_textures(&mut output, &texture_infos, |name| {
        images
            .remove(name)
