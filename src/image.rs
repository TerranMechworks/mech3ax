...
pub fn rgb888to565(src: Vec<u8>) -> Vec<u8> {
    ...
    loop {
        ...
        let green = LERP5[src[i + 1] as usize];
...
pub fn pal8to888(indices: Vec<u8>, palette: Vec<u8>) -> Vec<u8> {
...
pub fn pal8to888a(indices: Vec<u8>, palette: Vec<u8>, alpha: Vec<u8>) -> Vec<u8> {
    let src_len = indices.len();
