...
use mech3ax_api_types::anim::events::{
    ...
    CameraState, DetonateWeapon, Effect, Else, Elseif, Endif, Event, EventData, EventStart,
    FbfxColorFromTo, FbfxCsinwaveFromTo, FogState, If, InvalidateAnimation, LightAnimation,
    LightState, Loop, ObjectActiveState, ObjectAddChild, ObjectConnector, ObjectCycleTexture,
    ObjectDeleteChild, ObjectMotion, ObjectMotionFromTo, ObjectOpacityFromTo, ObjectOpacityState,
    ObjectRotateState, ObjectScaleState, ObjectTranslateState, ResetAnimation, Sound, SoundNode,
...
macro_rules! read {
    ($evt:ty, $read:ident, $anim_def:ident, $data_size:ident) => {{
        ...
        <$evt as EventRc>::read($read, $anim_def, $data_size)?
    }};
}
    ...
    while read.offset < end_offset {
        ...
        let data = match event_type {
            ...
            EventType::ObjectMotionSiScript => {
                ...
                let si_script =
                    object_motion_si_script::read_rc(read, anim_def, data_size, scripts)?;
