...
macro_rules! write {
    ($evt:ty, $write:ident, $anim_def:ident, $data:ident) => {{
        let size = <$evt as EventRc>::size($data);
        log::trace!(
        ...
        <$evt as EventRc>::write($data, $write, $anim_def)
    }};
}
...
fn write_event(
    ...
    match &event.data {
        ...
        EventData::ObjectMotionSiScript(data) => {
            ...
            object_motion_si_script::write_rc(write, anim_def, data, scripts)
        }
        ...
        EventData::FbfxCsinwaveFromTo(data) => write!(FbfxCsinwaveFromTo, write, anim_def, data),
        EventData::AnimVerbose(data) => write!(AnimVerbose, write, anim_def, data),
        EventData::DetonateWeapon(data) => write!(DetonateWeapon, write, anim_def, data),
        EventData::PufferState(_) => Err(assert_with_msg!("invalid event `PufferState` for RC")),
    }
    */
}
...
fn size_event(event: &Event, scripts: &[SiScript]) -> Option<u32> {
    ...
    match &event.data {
        ...
        EventData::ObjectMotionSiScript(data) => object_motion_si_script::size_rc(data, scripts),
        ...
        EventData::PufferState(_) => Some(0),
    }
    */
}
