...
macro_rules! write {
    ($evt:ty, $write:ident, $anim_def:ident, $data:ident) => {{
        let size = <$evt as EventPm>::size($data);
        log::trace!(
        ...
        <$evt as EventPm>::write($data, $write, $anim_def)
    }};
}
...
fn write_event(
    ...
    match &event.data {
        ...
        EventData::ObjectMotionSiScript(data) => {
            write!(ObjectMotionSiScript, write, anim_def, data)
        }
...
fn size_event(event: &Event) -> Option<u32> {
    match &event.data {
        ...
        EventData::ObjectMotionSiScript(inner) => inner.size(),
...
pub fn size_events(events: &[Event]) -> Option<u32> {
    let mut size = 0u32;
    for event in events {
        size = size.checked_add(EventHeaderC::SIZE)?;
        let event_size = size_event(event)?;
