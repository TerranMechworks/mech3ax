...
impl_as_bytes!(LightStateMwC, 120);
...
impl EventMw for LightState {
    #[inline]
    fn size(&self) -> Option<u32> {
        Some(LightStateMwC::SIZE)
    }

    fn read(read: &mut CountingReader<impl Read>, anim_def: &AnimDef, size: u32) -> Result<Self> {
        assert_that!("light state size", size == LightStateMwC::SIZE, read.offset)?;
        let state: LightStateMwC = read.read_struct()?;
        ...
        let translate = if flags.contains(LightFlags::AT_NODE) {
            let name = if state.node_index == index!(input) {
                ...
                anim_def.node_from_index(state.node_index, read.prev + 64)?
            };
        ...
        let color = if flags.contains(LightFlags::COLOR) {
            assert_color!("light state color", state.color, read.prev + 100)?;
            Some(state.color)
        } else {
            assert_that!(
                "light state color",
        ...
        let ambient = if flags.contains(LightFlags::AMBIENT) {
            assert_that!("light state ambient", 0.0 <= state.ambient <= 1.0, read.prev + 112)?;
            Some(state.ambient)
        } else {
            assert_that!("light state ambient", state.ambient == 0.0, read.prev + 112)?;
            None
        };

        let diffuse = if flags.contains(LightFlags::DIFFUSE) {
            assert_that!("light state diffuse", 0.0 <= state.diffuse <= 1.0, read.prev + 116)?;
            Some(state.diffuse)
        } else {
            assert_that!("light state diffuse", state.diffuse == 0.0, read.prev + 116)?;
            None
        };
