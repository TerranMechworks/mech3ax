...
#[test]
fn all_zero_index() {
    let err = assert_that!("foo", zero[3], 42).unwrap_err();
    assert_eq!(
        format!("{:#?}", err),
        "Expected `foo` to be zero, but byte 0 was 03 (at 42)"
    );

    let err = assert_that!("foo", zero [0, 3], 42).unwrap_err();
    assert_eq!(
        format!("{:#?}", err),
        "Expected `foo` to be zero, but byte 1 was 03 (at 43)"
    );

    let err = assert_that!("foo", zero [0, 255, 0], 42).unwrap_err();
    assert_eq!(
        format!("{:#?}", err),
        "Expected `foo` to be zero, but byte 1 was FF (at 43)"
    );
}
