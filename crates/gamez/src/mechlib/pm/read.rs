...
fn read_tree(
    ...
    let (data, model_index) = match node_info.node_class {
        NodeClass::Lod => {
            let lod = crate::nodes::lod::pm::read(read)?;
            let data = NodeData::Lod(lod);
            (data, IndexO::NONE)
        }
        NodeClass::Object3d => {
            let object3d = crate::nodes::object3d::read(read)?;
            let data = NodeData::Object3d(object3d);
            ...
            let model_index = if node_info.offset != 0 {
                ...
                let model_index = IndexO::from_usize(models.len())
        ...
        other => unreachable!("invalid mechlib node class {other:?}"),
    };
