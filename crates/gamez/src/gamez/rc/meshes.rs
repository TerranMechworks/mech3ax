...
use crate::mesh::rc::{
    ...
    write_mesh_info, MeshRcC, MESH_C_SIZE,
};
use log::trace;
...
pub(crate) fn read_meshes(
    ...
    for (mesh_index, expected_index) in mesh_indices.zeros() {
        let mesh: MeshRcC = read.read_struct_no_log()?;
...
pub(crate) fn size_meshes(offset: u32, meshes: &[MeshRc]) -> (u32, Vec<u32>) {
