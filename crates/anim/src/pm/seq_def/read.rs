...
use log::trace;
use mech3ax_anim_events::pm::read_events;
use mech3ax_api_types::anim::{AnimDef, ResetState, SeqActivation, SeqDef};
...
pub(crate) fn read_reset_state(
    ...
    // TODO
    log::info!("PM RESET PTR: {:08X} {:08X}", ptr, pointer);
    let length = reset_state.size;

    let events = read_events(read, length, anim_def)?;
