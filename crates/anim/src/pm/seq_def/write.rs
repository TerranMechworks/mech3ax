...
use log::trace;
use mech3ax_anim_events::pm::{size_events, write_events};
use mech3ax_api_types::anim::{AnimDef, ResetState, SeqActivation};
...
pub(crate) fn write_reset_state(
    ...
    let size = size_events(&reset_state.events)
    ...
    write_events(write, anim_def, &reset_state.events)?;
    Ok(())
}
