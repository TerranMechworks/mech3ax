...
static_assert_size!(DisplayCsC, 28);
...
pub fn read(read: &mut CountingReader<impl Read>, data_ptr: u32, index: usize) -> Result<Display> {
    debug!(
        "Reading display node data {} (cs, {}) at {}",
        index,
        DisplayCsC::SIZE,
        read.offset
    );
    let display: DisplayCsC = read.read_struct()?;
    trace!("{:#?}", display);
...
pub fn write(
    ...
    debug!(
        "Writing display node data {} (cs, {}) at {}",
        index,
        DisplayCsC::SIZE,
        write.offset
    );
    let display = DisplayCsC {
        origin_x: 0,
        origin_y: 0,
        resolution_x: display.resolution_x,
        resolution_y: display.resolution_y,
