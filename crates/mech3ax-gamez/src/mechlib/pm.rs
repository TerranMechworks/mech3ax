use crate::mesh::{read_mesh_data_pm, read_mesh_info_pm, write_mesh_data_pm, write_mesh_info_pm};
use mech3ax_api_types::{LodPm, MeshPm, ModelPm, NodePm, Object3dPm};
use mech3ax_common::io_ext::{CountingReader, CountingWriter};
use mech3ax_common::Result;
use mech3ax_nodes::{
    read_node_mechlib_pm, write_node_data_pm, write_node_info_pm, WrappedNodePm, WrapperPm,
};
...
fn read_node_and_mesh(
    ...
    meshes: &mut Vec<MeshPm>,
    mesh_ptrs: &mut Vec<i32>,
) -> Result<u32> {
    match read_node_mechlib_pm(read, nodes.len())? {
...
fn read_node_and_mesh_lod(
    ...
    lod.parent = if has_parent { 1 } else { 0 };
...
fn read_node_and_mesh_object3d(
    ...
    if object3d.mesh_index != 0 {
        ...
        let wrapped_mesh = read_mesh_info_pm(read, mesh_index)?;
        let mesh = read_mesh_data_pm(read, wrapped_mesh, mesh_index)?;
        meshes.push(mesh);
    } else {
...
pub fn read_model_pm(read: &mut CountingReader<impl Read>) -> Result<ModelPm> {
...
fn write_node_and_mesh(
    ...
    write_node_info_pm(write, &node, true, index)?;
    write_node_data_pm(write, &node, index)?;
    ...
    if let Some(mesh_index) = restore_index {
        ...
        write_mesh_info_pm(write, mesh, mesh_index)?;
        write_mesh_data_pm(write, mesh, mesh_index)?;
    }
...
pub fn write_model_pm(write: &mut CountingWriter<impl Write>, model: &mut ModelPm) -> Result<()> {
